...
Released binaries are built by GitHub Actions, not on a developer machine, so
every published artifact has a public build log you can read. That workflow uses
`build.bat` and the w64devkit release pinned in `flags.mk` — the same toolchain
the instructions tell you to use — and `build.bat` verifies that toolchain's
SHA-256 and Authenticode signature before extracting it.

The build is reproducible: the PE timestamp is zeroed
(`-Wl,--no-insert-timestamp`) and every flag comes from `flags.mk`. Building the
same version tag with the same w64devkit release must produce a binary whose
SHA-256 matches the one published with the release.

```powershell
Get-FileHash .\zdisplay-1.0.0-x64.exe -Algorithm SHA256
```

Each release also carries a `SHA256SUMS.txt` produced by that same run, listing
every asset.

Every artifact additionally carries a signed **provenance attestation**, which
records the commit, the workflow and the run that produced it. A matching hash
only proves two files are identical; the attestation proves where the file came
from, and is checked against a public transparency log rather than against a
...
If a hash does not match, do not run the file — and tell us through the private
channel above.
